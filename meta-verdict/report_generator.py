...
def generate_report(
    ...
    md_lines.extend([
        ...
        "| **小号集群调节分 (Score)** | 依据 7 天内新激活小号控盘度赋予的独立正向激励调节分 | 基础最高 `+1.50` 分，若触发穿透双共振最高 `+2.00` 分 |",
        "| **筹码建仓形态 (Pattern)** | 区分穿透与新号的三态分类 (双共振/纯新号/纯穿透) | `🔥双共振` 代表复合老鼠仓，`🌱纯新号` 代表突击潜伏盘 |",
        "",
        "### 2. 置信梯队与风控分类标准",
    ...
    if fresh_pool:
        ...
        for r in fresh_pool[:30]:
            ...
            pat = getattr(r, "sybil_pattern", "")
            if pat == "DUAL_RESONANCE":
                cat = "🔥 穿透+新号双共振"
                behavior = f"存在间接DEX穿透链路，且7天内激活 {cnt_7d} 个新号拿筹(控盘{pct_7d:.1f}%)，复合组织度极高"
                strategy = "高置信复合庄家盘 / 紧密跟踪资金拓扑与放量突破"
            elif pct_7d >= 30.0 and cnt_7d >= 10:
                cat = "🚨 重度小号集群控盘"
