...
| **E 钻石绞杀** | 机构控盘率≥90% + DEX真金率≥90% | 极低流通+真金吸筹 |
...
#### DEX真金率 (dex_verified_pct)
```
吸筹地址中 (dex_ratio≥0.5 OR gmgn_verified≥1) 的数量
/ 吸筹地址总数
× 100 (100进制)
```
...
| 问题 | 状态 | 优化方向 |
|------|------|----------|
| A→B→C 多跳转账导致 C 的 dex_ratio=0 | 已知 | 纳入 `dex_ratio_hop2` 字段 |
| 同主体多钱包未穿透 | 已知 | `entity_id` 聚簇共享 dex_ratio |
| 小地址噪声拉低 DEX率 | 已知 | 持仓加权替代数量占比 |
...
| 版本 | 日期 | 关键变更 |
|------|------|----------|
| **V8.2** | **2026-04-08** | master-scan 生产版：钻石绞杀区(模式E)、V8.2 三指标(机构控盘/隐庄/DEX真金率)、统一8列报告格式、去除黄色和Watchlist输出 |
