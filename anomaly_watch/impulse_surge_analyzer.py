...
class ImpulseSurgeAnalyzer:
    ...
    def _calc_token_surge(self, cur, t_addr: str, t_sym: str, t_chain: str) -> ImpulseSurgeResult:
        reasons = []
    ...
    def analyze(self) -> List[ImpulseSurgeResult]:
        ...
        for t in tokens:
            ...
            res = self._calc_token_surge(cur, t_addr, t_sym, t_chain)
            # 关键修改：保留所有代币计算结果，不因为 is_triggered 为 False 而抛弃！
            results.append(res)
    ...
    def analyze_single(self, token_address: str, chain: str = "bsc", token_symbol: str = "UNKNOWN") -> Optional[ImpulseSurgeResult]:
        ...
        conn.row_factory = sqlite3.Row
        cur = conn.cursor()
        t_addr = token_address.lower()
        res = self._calc_token_surge(cur, t_addr, token_symbol, chain)
        conn.close()
        return res
