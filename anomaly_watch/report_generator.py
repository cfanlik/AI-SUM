...
class AnomalyReportGenerator:
    ...
    def generate_report(self, token_results):
        ...
        content += "## 一、 🚨 重点关注：四大物理维度严谨核验与风控列表 (FAKE_ZERO_LIQUIDITY (> $10万标称 且 < $1万托管))\n\n"
        content += "> **物理风控断言**：绝对物理拆分四大维度（维1：API标称/Active TVL，维2：24h成交流水，维3：RPC实测沉淀本金）。零换手伪流动性诱多陷阱将在 Meta 仲裁中物理扣除 -40 分（物理筛查门槛: 维1标称 >= $100,000.00 且 维3实测托管 < $10,000.00）。\n\n"
