...
def ensure_penetration_table(sum_conn: sqlite3.Connection):
    ...
    sum_conn.execute("""
        CREATE TABLE IF NOT EXISTS dex_penetration_snapshots (
            ...
            avg_cost_usd REAL,             -- 庄家买入成本
            unrealized_pnl_pct REAL,       -- 庄家浮盈率
    ...
    for row in tokens_rows:
        ...
        flow_row = src_conn.execute("""
            SELECT SUM(swap_in_value) as raw_units,
                   AVG(CASE WHEN gmgn_buy_cost_usd > 0 THEN gmgn_buy_cost_usd END) as avg_cost,
        ...
        # 庄家浮盈率
        avg_cost = flow_row["avg_cost"] if flow_row else None
        unrealized_pnl_pct = ((price_usd - avg_cost) / avg_cost * 100.0) if (price_usd > 0 and avg_cost and avg_cost > 0) else None
...
def export_penetration_markdown(results: List[Dict[str, Any]], scan_time: str):
    ...
    lines.append(f"\n> **生成时间**：{scan_time} | 算法版本：damf_v7 (物理白盒防错版)")
    lines.append("> ⚠ `swap_in_value` 已无条件全表乘以 price_usd 折算为 USD，并做 reserve_usd×5 封顶截断。\n")
    lines.append("---")
    lines.append("\n## 🎯 一、 DEX 真实资金净流入与池子穿透排行 (Top 10)")
    lines.append("\n| 排名 | 代币 | 链 | 现价 | 真实 DEX 纯买入USD | 资金池深度 (LP) | 纯买入/LP 比率 | 庄家浮盈率 | 异常建仓诊断 |")
    lines.append("| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |")
    ...
    for r in sorted_results[:10]:
        ...
        lines.append(f"| **No.{idx}** | {r['token_symbol']} | `{r['chain']}` | ${r['price_usd']:.4f} | **{swap_str}** | ${r['reserve_usd']:,.2f} | **{r['swap_lp_ratio']:.1f}%** | **{pnl_str}** | {diag} |")
        idx += 1
